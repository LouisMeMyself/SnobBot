...
class SherpaPic:
    def __init__(self):
        ...
        with open("utils/Sherpa-logo.svg", "rb") as f:
            self.sherpaSVG = f.read().decode("utf-8")
        self.index = str(self.sherpaSVG).find("#F24E4D;}")
        self.sherpaSVG = list(self.sherpaSVG)
    ...
    def do_profile_picture(self, content):
        try:
            ...
            if self.hex_regex.match(new_color) is not None and len(new_color) == 6:
                self.sherpaSVG[self.index + 1: self.index + 7] = new_color
                svg2png("".join(self.sherpaSVG), write_to="utils/Sherpa-logo.png")
                return "Here is your personalized profile picture!", discord.File("utils/Sherpa-logo.png")
            return "Please write a HEX color or a RGB color. in these formats: '#00FFFF', '00FFFF', '0 255 255' or '0,255,255"
        except ValueError:
            return "Please write a HEX color or a RGB color. in these formats: '#00FFFF', '00FFFF', '0 255 255' or '0,255,255"
        except:
